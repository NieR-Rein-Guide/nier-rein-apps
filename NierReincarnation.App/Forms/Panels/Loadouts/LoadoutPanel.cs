...
{
    internal partial class LoadoutPanel : Panel
    {
        ...
        {
            return GetCurrentDeck()?.UserDeckActors.Where((_, i) => i != pos).Select(x => x?.Costume).Where(x => x != null).ToArray() ?? Array.Empty<DataOutgameCostumeInfo>();
        }

        internal DataWeaponInfo[] GetOtherDeckWeapons(int pos)
        {
            return GetCurrentDeck()?.UserDeckActors.Where((_, i) => i != pos).SelectMany(x => new[] { x?.MainWeapon, x?.SubWeapon01, x?.SubWeapon02 }).Where(x => x != null).ToArray() ?? Array.Empty<DataWeaponInfo>();
        }

        internal DataOutgameCompanionInfo[] GetDeckCompanions(int pos)
        {
            return GetCurrentDeck()?.UserDeckActors.Where((_, i) => i != pos).Select(x => x?.Companion).Where(x => x != null).ToArray() ?? Array.Empty<DataOutgameCompanionInfo>();
        }

        internal DataOutgameThoughtInfo[] GetDeckThoughts(int pos)
        {
            return GetCurrentDeck()?.UserDeckActors.Where((_, i) => i != pos).Select(x => x?.Thought).Where(x => x != null).ToArray() ?? Array.Empty<DataOutgameThoughtInfo>();
        }

        internal DataOutgameMemoryInfo[] GetDeckMemoirs(int pos)
        {
            return GetCurrentDeck()?.UserDeckActors.Where((_, i) => i != pos).SelectMany(x => x?.Memories).Where(x => x != null).ToArray() ?? Array.Empty<DataOutgameMemoryInfo>();
        }
