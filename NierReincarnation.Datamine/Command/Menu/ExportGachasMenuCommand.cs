...
public class ExportGachasMenuCommand : AbstractMenuCommand<ExportGachasCommandArg>
{
    ...
    public override async Task ExecuteAsync(ExportGachasCommandArg arg)
    {
        ...
        //    // Exclude banners that don't match the keyword
        //    if (!string.IsNullOrEmpty(arg.Keyword) && !gacha.Name.Contains(arg.Keyword, StringComparison.OrdinalIgnoreCase)) continue;

        //    // Exclude banners that don't match one of the specified types
        //    if (arg.GachaLabelTypes?.Count > 0 && !arg.GachaLabelTypes.Contains(gacha.GachaLabelType)) continue;

        //    // Process banner
        //    var gachaRates = await gachaContext.GetRates(gacha.GachaId);
        //    if (gachaRates is GachaRateBasic rates)
        //    {
        //        GachaModel existing = gachas.Find(x => x.GachaId == gacha.GachaId);

        //        if (existing != null)
        //        {
        //            gachas.Remove(existing);
        //        }
