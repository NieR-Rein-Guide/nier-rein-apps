...
public class BigHuntBattleContext : BaseContext
{
    ...
    public async Task<BigHuntBattleStatus> ExecuteBigHuntQuest(BigHuntQuestData quest, int bigHuntDeckNumber, SubjugationGrade grade)
    {
        ...
        var npcDecks = CalculatorQuest.GetWaveDataList(quest.Quest.QuestId, out var _);
    ...
    private static StartBigHuntQuestRequest GetStartBigHuntQuestRequest(BigHuntQuestData quest, int bigHuntDeckNumber)
    {
    ...
    private static FinishBigHuntQuestRequest GetFinishBigHuntQuestRequest(BigHuntQuestData quest, bool retire)
    {
    ...
    private static long[] GetDeckDamage(DataDeck[] userDecks, long totalDmg)
    {
    ...
    private static (int, long)[] GetBigHuntDamageInfo(DataDeck deck, long waveDmg, int knockdownModifierPermil)
    {
        var result = new (int, long)[3];

        var totalHits = 0;
        const int knockdownHitLimit = 20;
