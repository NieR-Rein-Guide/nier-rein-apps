...
namespace nier_rein_gui.Dialogs
{
    class VersionMaintenanceDialog : Modal
    {
        ...
        {
            var warningLabel = new Label {Caption = "There is either a new version or a maintenance."};
            var secondWarningLabel=new Label{Caption = $"Manually change the current version ({NierReincarnation.Core.UnityEngine.Application.Version}) before restarting the app,\nif a new version released!" };
        ...
        {
            var newVersion = await InputBox.ShowAsync("Change game version", "Game version", NierReincarnation.Core.UnityEngine.Application.Version, "Version e.g. 2.8.20");
            if (newVersion != null)
            {
